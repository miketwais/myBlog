...
// _this.$emit('on-ready', blobInfo.blob().size, blobInfo.blob())
    ...
    function uploadPic() { // 发送请求
        ...
        //切记一定要加上header的Authorization信息，耗费半天时间（针对axios.js请求不需要另外设置，因为axios.js中有统一设置）
        xhr.setRequestHeader('Authorization','bearer '+ localStorage.getItem("currentUser_token"))
        xhr.onload = function() {
